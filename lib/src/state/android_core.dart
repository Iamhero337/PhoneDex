...
/// Centralized reactive state store for all Android device telemetry
class AndroidCore {
  AndroidCore._();
  static final AndroidCore _instance = AndroidCore._();
  static AndroidCore get instance => _instance;

  final jarConnected = ValueNotifier(false);
  final apkConnected = ValueNotifier(false);
  final allConnected = ValueNotifier(false);
  final isReconnecting = ValueNotifier(false);
  final reconnectionMessage = ValueNotifier('');
  ...
  int volumeMusic = 0, volumeMusicMax = 15;
  int volumeRing = 0, volumeRingMax = 7;

  bool wifi = false, bluetooth = false, mobileData = false;
  bool airplane = false, mute = false, rotationLock = false;
  bool location = false, torch = false;
  String wifiName = '', bluetoothName = '';
  ...
  void updateFromMessage(Map<String, dynamic> data) {
    ...
    switch (data['type'] as String?) {
      ...
      case 'volume_update':
        ...
        if (streams != null) for (final s in streams) {
  ...
  void dispose() {
    jarConnected.dispose(); apkConnected.dispose(); allConnected.dispose();
    isReconnecting.dispose(); reconnectionMessage.dispose();
  }
}